...
export function createSorobanDomainsSDK(c) {
  const {
    ...
    SOROBAN_DOMAINS_REVERSE_REGISTRAR_CONTRACT_ID,
  } = env<{
    SOROBAN_RPC_URL: string;
    STELLAR_NETWORK: string;
    SOROBAN_DOMAINS_VAULTS_CONTRACT_ID: string;
    SOROBAN_DOMAINS_REVERSE_REGISTRAR_CONTRACT_ID: string;
  }>(c);
  return new SorobanDomainsSDK({
    ...
    reverseRegistrarContractId: SOROBAN_DOMAINS_REVERSE_REGISTRAR_CONTRACT_ID,
    defaultFee: "1000000",
    defaultTimeout: 60,
    simulationAccount:
